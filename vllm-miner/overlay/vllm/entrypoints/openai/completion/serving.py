...
class OpenAIServingCompletion(OpenAIServing):
    ...
    async def create_completion(
        ...
        for i, engine_input in enumerate(engine_inputs):
            ...
            # PoW: inject completion tracking IDs into extra_args
            # only when PoW payload is present. Copy to avoid
            # aliasing with the request's vllm_xargs dict across
            # multi-prompt sub-requests.
            if (isinstance(sampling_params, SamplingParams)
                    and sampling_params.extra_args
                    and "pow" in sampling_params.extra_args):
                sampling_params.extra_args = dict(
                    sampling_params.extra_args)
                sampling_params.extra_args["completion_id"] = \
                    request_id
                sampling_params.extra_args["request_item_id"] = \
                    request_id_item
