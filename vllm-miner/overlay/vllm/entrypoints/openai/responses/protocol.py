...
class ResponsesRequest(OpenAIBaseModel):
    ...
    extra_sampling_params: dict[str, Any] | None = Field(
        default=None,
        description=(
            "Backward-compatible alias for vllm_xargs used by older "
            "custom clients and proxy integrations."
        ),
    )
    ...
    def _build_extra_args(self) -> dict[str, Any]:
        ...
        if self.vllm_xargs:
            extra_args.update(self.vllm_xargs)
        if self.extra_sampling_params:
            extra_args.update(self.extra_sampling_params)
        return extra_args
    ...
    def to_sampling_params(
        ...
        extra_args: dict[str, Any] = self._build_extra_args()
