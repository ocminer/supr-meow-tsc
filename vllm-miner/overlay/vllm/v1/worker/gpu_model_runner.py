...
class GPUModelRunner(
    ...
    def __init__(
        ...
        # Sampler (with optional PoW RuntimeInfo).
        #
        # Contract for PoW images:
        #   - ImportError on `vllm.model_executor.runtime_info` is the only
        #     legitimate "skip RuntimeInfo" case. It means PoW shared-utils
        #     are not installed (stock vLLM build) and the sampler will run
        #     in non-PoW mode.
        #   - Any other failure (RuntimeError / dtype not in verifier
        #     vocabulary / corrupted model config) is FATAL — we refuse to
        #     start the worker rather than silently produce proofs that the
        #     verifier will reject. Misconfiguration must surface at boot.
        runtime_info = None
        try:
            from vllm.model_executor.runtime_info import RuntimeInfo
        except ImportError:
            pass  # PoW shared-utils not installed — expected in stock vLLM
        else:
            runtime_info = RuntimeInfo.from_configs(
