...
class InputBatch:
    def __init__(
        ...
        # PoW: per-request extra args (pow params, completion_id, etc.)
        self.extra_args_by_req: dict[str, dict[str, Any]] = {}
    ...
    def add_request(
        ...
        if sampling_params := request.sampling_params:
            ...
            if sampling_params.extra_args:
                self.extra_args_by_req[req_id] = sampling_params.extra_args
    ...
    def remove_request(self, req_id: str) -> int | None:
        ...
        self.extra_args_by_req.pop(req_id, None)
    ...
    def _make_sampling_metadata(self) -> SamplingMetadata:
        ...
        has_pow_reqs = any(
            "pow" in args for args in self.extra_args_by_req.values()
        )
        ...
        # Build per-batch-index extra_args from per-req-id storage.
        extra_args: dict[int, dict[str, Any]] | None = None
        if self.extra_args_by_req:
            extra_args_by_index: dict[int, dict[str, Any]] = {}
            for i, req_id in enumerate(self.req_ids[:num_reqs]):
                args_i = dict(self.extra_args_by_req.get(req_id, {}))
                args_i["req_id"] = req_id
                args_i["pad_id"] = self.vocab_size
                extra_args_by_index[i] = args_i
            extra_args = extra_args_by_index or None
